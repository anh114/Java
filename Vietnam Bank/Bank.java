...
public class Bank {
    ...
    public boolean isCustomerExisted(String customerId) {
        for (int i = 0; i < customers.size(); i++) {
            ...
            if (customers.get(i).getCustomerId()== Long.parseLong(customerId)) {
    ...
    public void addAccount(String customerId, Account account) {
        for(Customer cus: customers) {
            ...
            if(cus.getCustomerId()== Long.parseLong(customerId)) {
                //add account vao trong Customer
                cus.addAccount(account);
            } else {
                System.out.println("Khong co khach hang nay trong Bank");
            }
        }
